          ...
          <div class="settings-list">
            <label v-for="setting in categorySettings[setCat.id]" :key="setting.id" class="setting-item" @click="showAuthMessage">
              <div class="setting-checkbox-wrapper">
                <input type="checkbox" class="setting-checkbox-input" disabled @click.prevent />
...
.hero { background: #1f2937; color: #fff; border-bottom-left-radius: 1.25vw; border-bottom-right-radius: 1.25vw; padding-bottom: 3.125vw; }
...
.decor-hero-left-top, .decor-hero-right-bottom {
  height: 2.6042vw;
  pointer-events: none;
}
...
.card:hover .card-img {
  ...
  box-shadow: 0 .5208vw .9375vw rgba(0,0,0,.12);
}
...
.program-checkbox-input:checked + .program-checkbox-label {
  background: #facc15;
  border-color: #facc15;
}
.program-checkbox-input:checked + .program-checkbox-label::after {
  content: ''; position: absolute;
  left: 0.4vw; top: 0.15vw;
  width: 0.4vw;
  height: 0.7vw;
  border: solid #1f2937;
...
@media (max-width: 768px) {
  ...
  .warning-decor-left { width: 80px; height: auto; }
  .warning-decor-right { width: 70px; height: auto; }
  .warning-title {
    font-size: 16px;
    margin-bottom: 12px;
    line-height: 1.4;
