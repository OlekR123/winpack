...
function editCategory(cat) {
  ...
  formData.value = { type: cat.type || 'program', title: cat.title || '' };
...
async function saveCategory() {
  ...
  try {
    ...
    if (showEditModal.value) {
      res = await adminApi.updateCategory(editingId.value, formData.value);
    } else {
...
.modal { background: #fff; padding: 2.6042vw 3.125vw; border-radius: 1.25vw; box-shadow: 0 0.5208vw 2.0833vw rgba(0, 0, 0, 0.15); width: 26.0417vw; max-width: 90vw; max-height: 85vh; overflow-y: auto; }
