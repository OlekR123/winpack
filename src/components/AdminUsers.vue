<template>
  <div class="admin-page">
    ...
    <div class="table-container">
      <table class="admin-table">
        ...
        <tr v-for="user in users" :key="user.id">
          ...
          <td>
            <div class="actions">
              <!-- Свою роль менять нельзя — кнопку на своей строке не показываем -->
              <button
                  v-if="user.id !== authStore.userId"
                  class="btn-edit"
                  @click="editUser(user)"
              >Изменить роль</button>
              <!-- Себя удалить нельзя — кнопку на своей строке не показываем -->
              <button
                  v-if="user.id !== authStore.userId"
                  class="btn-delete"
                  @click="deleteUser(user.id)"
              >Удалить</button>
              <span v-if="user.id === authStore.userId" class="self-note">это вы</span>
            </div>
          </td>
        </tr>
        </tbody>
    ...
    <div v-if="showAddModal" class="modal-overlay" @mousedown.self="closeModals">
    ...
    <div v-if="showEditModal" class="modal-overlay" @mousedown.self="closeModals">
      <div class="modal" @click.stop>
        ...
        <div class="modal-form">
          ...
          <button class="form-btn-submit" @click="updateRole">Сохранить</button>
          <button class="form-btn-cancel" @click="closeModals">Отмена</button>
        </div>
      </div>
    </div>
...
const formData = ref({ email: '', password: '', role: 'user' });
const editingUser = ref({ id: null, email: '', role: '' });
...
function openAddModal() {
  formData.value = { email: '', password: '', role: 'user' };
  showAddModal.value = true;
}

function editUser(user) {
  editingUser.value = { id: user.id, email: user.email, role: user.role_name || 'user' };
...
function closeModals() {
  ...
  formData.value = { email: '', password: '', role: 'user' };
  editingUser.value = { id: null, email: '', role: '' };
}
...
async function addUser() {
  ...
  try {
    ...
    if (!res.ok) { const error = await res.json(); alert(error.error || 'Ошибка добавления'); return; }
    await loadUsers();
    closeModals();
  } catch (e) {
...
async function updateRole() {
  if (!editingUser.value.role) { alert('Выберите роль'); return; }
  try {
    const res = await adminApi.updateUserRole(editingUser.value.id, editingUser.value.role);
    if (!res.ok) { const error = await res.json(); alert(error.error || 'Ошибка обновления'); return; }
    await loadUsers();
    closeModals();
  } catch (e) {
...
function formatDate(dateStr) {
  if (!dateStr) return '-';
  return new Date(dateStr).toLocaleDateString('ru-RU', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
}
</script>
...
.self-note { color: #9ca3af; font-size: 1.1vw; font-style: italic; white-space: nowrap; }
