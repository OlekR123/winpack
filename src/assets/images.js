export const images = {
    logo: 'https://i.postimg.cc/fW67xSc7/Icon.png',
    profile: 'https://i.postimg.cc/fbpy6mT8/Profile.png',
    logout: 'https://i.postimg.cc/k5KPgby3/Exit.png',
    decorHeroLeftTop: 'https://i.postimg.cc/YS52X7bZ/left-dots-header.png',
    decorHeroRightBottom: 'https://i.postimg.cc/mgqTDRf7/right_dots_header.png',
    warningDotsLeft: 'https://i.postimg.cc/G9wkr632/left-dots-wor-set.png',
    warningDotsRight: 'https://i.postimg.cc/vmH2pxSp/right-dots-wor-set.png',
    qrcode: 'https://i.postimg.cc/J0JpbNG0/qrcode.png',

    stats: 'https://i.postimg.cc/vTppy86x/stats.png',
    category: 'https://i.postimg.cc/fyx6wn70/category.png',
    program: 'https://i.postimg.cc/8PS2ZcNg/program.png',
    settings: 'https://i.postimg.cc/nhdNV17f/settings.png',
    user: 'https://i.postimg.cc/RhzDppr1/user.png'
};

export const drivers = [
    {
        name: 'Nvidia',
        url: 'https://www.nvidia.com/Download/index.aspx',
        img: 'https://i.postimg.cc/SRqMvCg8/Nvidia.png'
    },
    {
        name: 'AMD',
        url: 'https://www.amd.com/en/support',
        img: 'https://i.postimg.cc/VkHXkSkG/AMD.png'
    },
    {
        name: 'Intel',
        url: 'https://www.intel.com/content/www/us/en/download-center/home.html',
        img: 'https://i.postimg.cc/sxwvH8Bw/Intel.png'
    }
];